...
function setAutoMode() { autoMode.value = true }
function setManualMode() { autoMode.value = false }
...
onMounted(() => {
  ...
  clockTimer = setInterval(() => { now.value = DateTime.now().setZone(timeZone) }, 1000)
})
...
watch(timeLineTop, (top) => {
  if (top === null || !scrollEl.value) return
  const el = scrollEl.value
  const target = top - el.clientHeight * 0.4
    ...
    >
      <div class="schedule-inner">
        ...
        <div class="stages-area">
          <!-- Current time line (spans all stages) -->
          <div
            v-if="timeLineTop !== null"
            class="time-line"
            :style="{ top: `${timeLineTop}px` }"
...
.ctrl-btn.active { background: rgba(255,145,77,0.18); border-color: #ff914d; color: #ff914d; }
...
.talk-card:hover { transform: scale(1.01); z-index: 10; }
