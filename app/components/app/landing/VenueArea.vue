...
const uniqueTalks = computed(() => {
  ...
  return talks.value.filter((t: any) => {
...
function formatTime(dateTime: string) {
  return DateTime.fromISO(dateTime, { zone: 'UTC' }).setZone('Europe/Berlin').toFormat('HH:mm')
}
      ...
      <div v-if="uniqueTalks.length" class="mt-4 space-y-2">
        ...
        <ul class="space-y-1.5">
          ...
          >
            ...
            />
            <span class="text-sm text-muted">{{ talk.title }}</span>
          </li>
        </ul>
      </div>
